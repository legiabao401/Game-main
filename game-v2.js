...
async function joinOnlineRoom(){const input=document.getElementById('v2RoomCode'),error=document.getElementById('v2RoomError'),code=input?.value.trim().toUpperCase();if(!/^[A-Z0-9]{6}$/.test(code))return error.textContent='Mã phòng phải có đúng 6 ký tự.';error.textContent='Đang tham gia…';try{const ref=DB_ROOMS.child(code);let reason='';const result=await ref.transaction(room=>{if(!room){reason='Không tìm thấy phòng.';return}if(room.status!=='lobby'){reason='Trận đấu đã bắt đầu.';return}room.players=room.players||{};if(!room.players[G.myPlayerId]&&Object.keys(room.players).length>=6){reason='Phòng đã đủ 6 người.';return}room.players[G.myPlayerId]=roomPlayerPayload();return room});if(!result.committed)throw new Error(reason||'Không thể tham gia phòng.');connectRoom(code,result.snapshot.val()?.hostId===G.myPlayerId)}catch(e){error.textContent=e.message||friendlyFirebaseError(e)}}

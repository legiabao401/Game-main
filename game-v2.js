...
async function login(){
  ...
  try{
    ...
    try{credential=await AUTH.signInWithEmailAndPassword(email,secret)}catch(signError){if(!/user-not-found|invalid-credential|invalid-login-credentials|internal-error/.test(signError?.code||''))throw signError;try{credential=await AUTH.createUserWithEmailAndPassword(email,secret);created=true}catch(createError){if(createError?.code==='auth/email-already-in-use')throw Object.assign(new Error('Sai mật khẩu'),{code:'auth/wrong-password'});throw createError}}

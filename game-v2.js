...
const FIREBASE_PLAYERS_REST='https://treasure-party-mln122-bao401-default-rtdb.asia-southeast1.firebasedatabase.app/treasure_hunt_v2/players';
const FLOOR_THEMES=[
...
let profile=loadProfile(), run=null, modal=null, toastTimer=null, questionTimer=null, leaderboard=[],lastSyncWarning=0;
const AUTH=firebase.auth(),DB_ROOMS=DB.ref('treasure_party_rooms_v1'),DB_LEADERBOARD=DB.ref('treasure_hunt_v2/leaderboard'),DB_NAME_INDEX=DB.ref('treasure_hunt_v2/nameIndex');
...
async function readPlayersResilient(){
  let sdkError=null;
  try{
    if(!G.myPlayerId)throw new Error('Chưa xác định tài khoản');
    const snap=await Promise.race([DB_PLAYERS.child(G.myPlayerId).once('value'),timeoutAfter(9000,'Firebase SDK')]);
    const player=snap.val();return player?{[G.myPlayerId]:player}:{}
  }catch(e){sdkError=e;console.warn('Firebase SDK read failed, trying REST:',e)}
  try{
    const token=await AUTH.currentUser?.getIdToken();if(!token)throw new Error('Chưa xác thực Firebase');
    const response=await Promise.race([fetch(`${FIREBASE_PLAYERS_REST}/${encodeURIComponent(G.myPlayerId)}.json?auth=${encodeURIComponent(token)}`,{cache:'no-store'}),timeoutAfter(9000,'Firebase REST')]);
    if(!response.ok)throw new Error(`Firebase REST HTTP ${response.status}`);
    const player=await response.json();return player?{[G.myPlayerId]:player}:{}
  }catch(restError){
...
async function writePlayerResilient(playerId,payload,replace=false){
  if(AUTH.currentUser?.uid!==playerId)throw new Error('Phiên đăng nhập không khớp tài khoản');
  try{
  ...
  const response=await Promise.race([fetch(`${FIREBASE_PLAYERS_REST}/${encodeURIComponent(playerId)}.json?auth=${encodeURIComponent(token)}`,{method:replace?'PUT':'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),timeoutAfter(9000,'Ghi Firebase REST')]);
...
function showModeMenu(){cleanupRoomListener();onlineRoom={code:'',ref:null,data:null,host:false,listening:false};cleanupExpiredRooms();openModal(`<div class="v2-end"><div class="trophy">🎲</div><h2>CHỌN CHẾ ĐỘ PARTY</h2><p>Bản đồ 12×12 · tối đa 6 người</p><div class="v2-mode-grid"><button class="v2-mode-card" onclick="V2.startBots()"><span>🤖</span><strong>Đấu với máy</strong><small>Bạn và 3 bot · chơi ngay</small></button><button class="v2-mode-card online" onclick="V2.createRoom()"><span>🌐</span><strong>Tạo phòng Online</strong><small>Nhận mã để mời tối đa 5 người</small></button></div><div class="v2-join-box"><b>Tham gia bằng mã phòng</b><div><input id="v2RoomCode" maxlength="6" placeholder="VD: A7K9PQ" oninput="this.value=this.value.toUpperCase().replace(/[^A-Z0-9]/g,'')"><button class="v2-btn gold" onclick="V2.joinRoom()">Tham gia</button></div><small id="v2RoomError"></small></div><button class="v2-btn" style="width:100%;margin-top:12px" onclick="V2.logout()">Đăng xuất tài khoản</button></div>`,{type:'mode',locked:true})}
function startBots(){closeModal(true);createRun()}
async function cleanupExpiredRooms(){if(!AUTH.currentUser)return;try{const snap=await DB_ROOMS.once('value'),cutoff=Date.now()-6*60*60*1000,tasks=[];snap.forEach(roomSnap=>{const room=roomSnap.val();if(room?.hostId===G.myPlayerId&&(room.createdAt||0)<cutoff)tasks.push(roomSnap.ref.remove())});await Promise.all(tasks)}catch(e){console.warn('Room cleanup skipped:',e)}}
async function createOnlineRoom(){const error=document.getElementById('v2RoomError');if(error)error.textContent='Đang tạo phòng…';try{if(AUTH.currentUser?.uid!==G.myPlayerId)throw new Error('Phiên đăng nhập đã hết hạn.');let code,ref,exists=true;for(let i=0;i<8&&exists;i++){code=roomCode();ref=DB_ROOMS.child(code);exists=(await ref.once('value')).exists()}if(exists)throw new Error('Chưa tạo được mã phòng duy nhất. Vui lòng thử lại.');await ref.set({code,hostId:G.myPlayerId,status:'lobby',maxPlayers:6,createdAt:firebase.database.ServerValue.TIMESTAMP,players:{[G.myPlayerId]:roomPlayerPayload()}});connectRoom(code,true)}catch(e){if(error)error.textContent=friendlyFirebaseError(e)}}
async function joinOnlineRoom(){const input=document.getElementById('v2RoomCode'),error=document.getElementById('v2RoomError'),code=input?.value.trim().toUpperCase();if(!/^[A-Z0-9]{6}$/.test(code))return error.textContent='Mã phòng phải có đúng 6 ký tự.';error.textContent='Đang tìm phòng…';try{if(AUTH.currentUser?.uid!==G.myPlayerId)throw new Error('Phiên đăng nhập đã hết hạn.');const ref=DB_ROOMS.child(code);const serverSnapshot=await ref.once('value'),serverRoom=serverSnapshot.val();if(!serverRoom)throw new Error('Không tìm thấy phòng. Hãy kiểm tra lại mã 6 ký tự.');if(serverRoom.status!=='lobby')throw new Error('Trận đấu đã bắt đầu hoặc phòng đã kết thúc.');const serverPlayers=serverRoom.players||{};if(!serverPlayers[G.myPlayerId]&&Object.keys(serverPlayers).length>=6)throw new Error('Phòng đã đủ 6 người.');error.textContent='Đang tham gia…';const result=await ref.child(`players/${G.myPlayerId}`).transaction(current=>current||roomPlayerPayload());if(!result.committed)throw new Error('Không thể tham gia phòng. Vui lòng thử lại.');connectRoom(code,serverRoom.hostId===G.myPlayerId)}catch(e){error.textContent=e?.message&&/Không tìm thấy|đã bắt đầu|đã kết thúc|đã đủ|hết hạn|Không thể tham gia/.test(e.message)?e.message:friendlyFirebaseError(e)}}
function connectRoom(code,isHost){cleanupRoomListener();onlineRoom={code,ref:DB_ROOMS.child(code),data:null,host:isHost,listening:true};onlineRoom.ref.on('value',snap=>{const data=snap.val();if(!data){toast('Phòng đã đóng.');showModeMenu();return}onlineRoom.data=data;onlineRoom.host=data.hostId===G.myPlayerId;handleRoomData(data)},e=>reportSyncFailure('Mất kết nối với phòng',e));onlineRoom.ref.child(`players/${G.myPlayerId}`).onDisconnect().remove().catch(e=>reportSyncFailure('Không thiết lập được trạng thái phòng',e))}
function handleRoomData(data){if(data.status==='lobby'){const ids=Object.keys(data.players||{});if(ids.length&&!data.players?.[data.hostId])onlineRoom.ref.child('hostId').transaction(h=>h===data.hostId?ids[0]:h).catch(e=>reportSyncFailure('Không thể chuyển chủ phòng',e));renderOnlineLobby(data);return}if(data.status==='playing'){if(!run?.party?.online){closeModal(true);createRun({online:true,roomCode:data.code,roomData:data})}const ids=Object.keys(data.players||{});if(run?.party?.online)run.party.players=run.party.players.filter(p=>ids.includes(p.id));applyOnlineStates(data.states||{});if(run?.party?.online){renderAll();const states=data.states||{};if(ids.length&&ids.every(id=>states[id]?.finished))onlineRoom.ref.child('status').transaction(s=>s==='playing'?'ceremony':s).catch(e=>reportSyncFailure('Không thể kết thúc trận',e))}}else if(data.status==='ceremony')beginOnlineCeremony(data)}
...
async function startOnlineRoom(){const data=onlineRoom.data,players=Object.keys(data?.players||{});if(!onlineRoom.host||players.length<2)return;try{createRun({online:true,roomCode:onlineRoom.code,roomData:data});const states={};run.party.players.forEach(p=>states[p.id]=onlineState(p));await onlineRoom.ref.update({status:'playing',board:run.board,bonusIds:shuffle(PARTY_BONUSES.map(b=>b.id)).slice(0,3),states,startedAt:firebase.database.ServerValue.TIMESTAMP});closeModal(true)}catch(e){run=null;renderOnlineLobby(data);reportSyncFailure('Không thể bắt đầu trận',e)}}
async function leaveOnlineRoom(){const ref=onlineRoom.ref,status=onlineRoom.data?.status;if(ref){cleanupRoomListener();try{if(onlineRoom.host&&status==='lobby')await ref.remove();else await ref.child(`players/${G.myPlayerId}`).remove()}catch(e){reportSyncFailure('Không thể rời phòng sạch sẽ',e)}}showModeMenu()}
...
function syncOnlinePlayer(extra={}){if(!run?.party?.online||!onlineRoom.ref)return Promise.resolve();syncHumanParty();return onlineRoom.ref.child(`states/${G.myPlayerId}`).update(onlineState(humanParty(),extra)).catch(e=>reportSyncFailure('Không thể đồng bộ lượt chơi',e))}
...
function loginScreen(){openModal(`<div class="v2-end"><div class="trophy">🎲</div><h2>TREASURE PARTY</h2><p>Đăng nhập an toàn bằng Firebase để chơi với bot hoặc mời tối đa 5 người bạn.</p><input id="v2Name" maxlength="20" placeholder="Tên người chơi" autocomplete="username" style="width:100%;margin-top:14px;padding:12px;border-radius:10px;border:1px solid #405171;background:#0b111e;color:white"><input id="v2Pass" type="password" maxlength="50" placeholder="Mật khẩu" autocomplete="current-password" style="width:100%;margin-top:8px;padding:12px;border-radius:10px;border:1px solid #405171;background:#0b111e;color:white"><div id="v2LoginError" style="color:#fca5a5;font-size:.78rem;margin-top:7px"></div><button class="v2-btn gold" style="width:100%;margin-top:10px" onclick="V2.login()">Đăng nhập / Tạo tài khoản</button></div>`,{type:'login',locked:true});setTimeout(()=>document.getElementById('v2Name')?.focus(),50)}
function normalizedAccountName(name){return name.trim().toLocaleLowerCase('vi-VN')}
async function accountAddress(name){const nameKey=await hashPassword(normalizedAccountName(name));return{nameKey,email:`${nameKey}@players.treasure-party.invalid`}}
function authPassword(password){return `TreasureParty::${password}`}
function authErrorMessage(e){const code=e?.code||'';if(/wrong-password|invalid-credential|invalid-login-credentials/.test(code))return 'Sai mật khẩu cho tên này.';if(/operation-not-allowed/.test(code))return 'Firebase Authentication chưa bật phương thức Email/Password.';if(/too-many-requests/.test(code))return 'Đăng nhập tạm khóa do thử quá nhiều lần. Hãy chờ rồi thử lại.';if(/network-request-failed/.test(code))return 'Không kết nối được Firebase Authentication.';return friendlyFirebaseError(e)}
async function createAuthAccount(email,password){const apiKey=firebase.app().options.apiKey,response=await fetch(`https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=${encodeURIComponent(apiKey)}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email,password,returnSecureToken:true})}),data=await response.json().catch(()=>({}));if(response.ok)return;if(data?.error?.message==='EMAIL_EXISTS')throw Object.assign(new Error('Sai mật khẩu'),{code:'auth/wrong-password'});throw Object.assign(new Error(data?.error?.message||`HTTP ${response.status}`),{code:'auth/create-failed'})}
function loadPlayerIntoSession(player,user,name){G.myPlayerId=user.uid;G.myPlayerName=player?.name||name;G.cups=player?.cups||0;G.totalMoves=player?.totalMoves||0;if(Number.isFinite(player?.expeditionGold))profile.bankGold=player.expeditionGold;if(Number.isFinite(player?.expeditionWins))profile.stats.wins=player.expeditionWins;saveIdentity();localStorage.setItem(SAVE_KEY,JSON.stringify(profile))}
async function login(){
  ...
  err.textContent='Đang xác thực an toàn…';
  try{
    await AUTH.setPersistence(firebase.auth.Auth.Persistence.LOCAL);const{nameKey,email}=await accountAddress(name),secret=authPassword(pw);let credential,created=false;
    try{credential=await AUTH.signInWithEmailAndPassword(email,secret)}catch(signError){if(!/user-not-found|invalid-credential|invalid-login-credentials|internal-error/.test(signError?.code||''))throw signError;await createAuthAccount(email,secret);created=true;credential=await AUTH.signInWithEmailAndPassword(email,secret)}
    const user=credential.user,ownRef=DB_PLAYERS.child(user.uid),ownSnap=await ownRef.once('value');let player=ownSnap.val();
    if(!player){const indexSnap=await DB_NAME_INDEX.child(nameKey).once('value'),index=indexSnap.val();if(index?.playerId){const legacyRef=DB_PLAYERS.child(index.playerId),legacySnap=await legacyRef.once('value'),legacy=legacySnap.val(),hashed=await hashPassword(pw);if(!legacy||legacy.authEmail!==email||legacy.password&&legacy.password!==hashed){if(created)await user.delete();else await AUTH.signOut();throw Object.assign(new Error('Sai mật khẩu'),{code:'auth/wrong-password'})}player={...legacy,name:legacy.name||name,playerId:user.uid,authEmail:email,nameKey};delete player.password;const rank={name:player.name,playerId:user.uid,cups:player.cups||0,totalMoves:player.totalMoves||0,expeditionGold:player.expeditionGold||0,expeditionWins:player.expeditionWins||0,lastActive:firebase.database.ServerValue.TIMESTAMP,online:true};await DB.ref().update({[`treasure_hunt_v2/players/${user.uid}`]:player,[`treasure_hunt_v2/players/${index.playerId}`]:null,[`treasure_hunt_v2/nameIndex/${nameKey}`]:null,[`treasure_hunt_v2/leaderboard/${user.uid}`]:rank})}else{player={name,playerId:user.uid,authEmail:email,nameKey,cups:0,totalMoves:0,expeditionGold:0,expeditionWins:0,online:true,lastActive:firebase.database.ServerValue.TIMESTAMP};await ownRef.set(player);await DB_LEADERBOARD.child(user.uid).set({name,playerId:user.uid,cups:0,totalMoves:0,expeditionGold:0,expeditionWins:0,online:true,lastActive:firebase.database.ServerValue.TIMESTAMP})}}
    loadPlayerIntoSession(player,user,name);listenRanks();closeModal(true);showModeMenu();syncCloud()
  }catch(e){console.error('Treasure Expedition login error:',e);err.textContent=authErrorMessage(e)}
}
